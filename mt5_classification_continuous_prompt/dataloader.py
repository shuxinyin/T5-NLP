...
class BatchTextCall(object):
    ...
    def __init__(self, tokenizer, max_len=64, label_len=4, use_prompt=True, num_tokens=20, class_num=10):
    ...
    def __call__(self, batch):
        ...
        labels = torch.cat([torch.full((batch_size, self.num_tokens - 10), -100),
                            labels], 1)
        labels[labels == 0.] = -100
        labels[labels == 1.] = 10
        # print(labels)

        return inputs, labels.to(torch.long), batch_true_label
...
if __name__ == "__main__":
    ...
    for text, label, batch_true_label in text_dataloader:
        # print(text)
        print(label.shape)
        # print(tokenizer.batch_decode(label['input_ids'], skip_special_tokens=True))
        # text = text.to(device)
        break
